...
def test_the_same_id_in_a_different_package_is_red(tmp_path):
    """Keying on the id alone would call this pre-existing. It is not."""
    b = report(tmp_path, "b", [("gobinary", "stdlib", "CVE-1")])
    i = report(tmp_path, "i", [("gobinary", "stdlib", "CVE-1"), ("os-pkgs", "curl", "CVE-1")])
    result = run(b, i)
    assert result.returncode == 1
